			...
			<div id="Content">
				<xsl:choose>
					...
					<xsl:when test="./activity[@type='test suite' or @type='test case' or @type='repeat container']">
						...
						<p> 
							<xsl:choose>
							<xsl:when test="./activity[@type='test suite']"><xsl:value-of select ="./activity/detail" disable-output-escaping="yes"/></xsl:when>
							<xsl:otherwise> 
...
<xsl:template name="ActivityDescription">
	...
	<i class="description"> <xsl:value-of select="substring($detailstext,0,50)"/>
